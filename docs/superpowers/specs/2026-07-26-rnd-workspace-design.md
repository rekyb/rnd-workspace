...
**Committed:** `ui/tokens.css`, `ui/tokens.json`, `ui/components.css`. A shipped web
...
2. **No raw style values** — no hex literal and no `px` literal outside `ui/tokens.css`
...
`ui/` layout, the overlay rule, the disclosure boundary, the five `check-prototype`
...
- `/sync-tokens --check` exits 0 immediately after a sync, and exits non-zero with a
  readable diff when any byte of `ui/tokens.css` is altered.
...
- `/design-prototype [project]` authors multi-file source in `src/` against the PRD and
  the `ui/` library; Principal Designer Mode T gate.
...
| Anonymous clone access is revoked upstream | Sync fails with a clear message; the committed `ui/` files keep working, only refresh is blocked |
