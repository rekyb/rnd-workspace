...
>
> **Four corrections this cycle forces, each recorded where the false statement was rather than
> quietly overwritten.** One claim — "there is no rail content to migrate or delete at narrow width" —
> had propagated to four sites, and Slice 16 puts two content slots in a rail and makes it false at
> every one: **§11**'s narrow-width paragraph, **§14**'s layout-F8 entry, and **both** §2.1 rows that
> leaned on it (layout-F5, which used it to answer F8's narrow-width version trivially, and
> layout-F8, which used it as a second reason for its disposition). **No disposition changes at any
> of the four** — the migrate-or-delete rule stays unadopted, because a rule the evidence does not
> support stays unsupported whether or not there is something for it to govern — but the reasoning
> that referenced an empty rail is corrected rather than left to read as still true.
>
> *The two §2.1 rows were missed in this revision's first pass and corrected on verification the same
> day. Recorded because §2.1 is the coverage table: a stale reason in a disposition row is the exact
> failure the declare-or-block rule exists to prevent, and the miss is worth a line so the next
> reader knows the table was audited rather than assumed.*
>
...
| A phone home carries three to five destinations, each with a text label; the one platform exceeding five drops its labels **and** adds an overflow | Layout & IA, **F7** | **Hypothesis.** Four iOS platforms. It converges with published three-to-five guidance `[R8]`, but that is **archived Material Design 1 guidance for native Android apps**, is a normative specification rather than a study, and plausibly shares one ancestry with the four apps — so the corroboration is convergent, not independent. One benchmarked platform is a published counter-instance at six | Slice 14: the narrow Learning Home exposes **at most five labelled destinations**. **The top level is four** — the families Home, Learn, Portfolio, Work — so the budget holds with **no demotion**. *(Updated 2026-07-30. This previously read "the prototype's home navigation is four" and called the production inheritance undecided. Both are now settled by §6.2 entry condition 5: production inherits the real eleven **reorganized into these four families**, so the four is no longer a property of the prototype alone and the eleven-item flat navigation the asymmetry warned about is no longer the alternative. What survives unchanged is that the budget counts **top-level** destinations; the seven inside family panels sit one level down and one of the four still routes to a surface §14 does not build.)* |
...
| F# | Finding | Disposition | Where / why |
|---|---|---|---|
| F1 | The cardinality device — a navigation needs a structural rule, not a shorter list | **Deferred** | §14 — navigation information architecture is a steady-state decision and this PRD builds first-run only. **The deferral has a named consequence rather than a shrug**, and that consequence is now discharged. ~~the prototype's home carries four destinations and the production home carries eleven, flat, ungrouped… that question is now an owned §15 decision~~ **Settled 2026-07-30:** production inherits the real **eleven reorganized into four top-level families**, which **partially reopens this deferral** — a purpose-based grouping *is* the structural device this finding recommends, so what remains deferred is only the steady-state home's full device decision, not the first-run navigation. See §15. **The study's own caveat still binds the remainder:** ten of the eleven destinations were never opened, so the family assignment is a judgement from labels and is carried as unvalidated with a card-sort gate, never as adopted |
...
| F5 | Progress separated by permanence | **Deferred** | §14 — the separation needs a detailed record to separate *from*, and points, streaks, and achievements are already non-goals, so the first-run home has exactly one progress affordance by construction. The finding's own clean-fit rate is **3 of 7**, and its behavioural-goal-horizon half needs a steady-state home. ~~**F8's narrow-width version is answered trivially here for the same reason:** there is no rail content to migrate or delete~~ **Corrected 2026-07-30 by Cycle 4:** Slice 16 puts two content slots in a rail, so F8's narrow-width version is no longer answered trivially. It is answered explicitly, in the four §11.1 disposition rows Slice 16 adds. **This row's own disposition is unchanged** — the permanence separation still needs a detailed record to separate from, and that argument never depended on the rail |
...
| F8 | When the rail disappears, the signal migrates or dies | **Adopted, structural half only** | Slice 14 adopts the shell-versus-per-view claim (four web platforms). **The migrate-or-delete rule is not adopted** — the study labels it hypothesis-generating from two opposite cases. ~~and this PRD has no rail content for it to govern~~ **Corrected 2026-07-30 by Cycle 4:** that second reason is now false, because Slice 16 puts two content slots in a rail. **The disposition is unchanged and the first reason is now the only reason** — an unadopted rule stays unadopted whether or not there is something for it to govern. See the fuller statement in §14. §2 records that this reading is narrower than the source's blanket caveat |
...
2. The Learning Home shell's navigation treatment has settled. **Met 2026-07-30** — the sidebar was
   ...
   §6.2 entry condition 5 records. Cycle 4 arranges content *beside* the content column, so it could
   not have begun against a shell still changing shape; that shell is now fixed.
3. The §15 navigation-inheritance decision has landed, for the same reason §6.2 names it: the
   validation instrument below is a first-click test, and a first-click test cannot be run against a
   home whose destination set is unsettled. **Met 2026-07-30** — settled as the real eleven
   reorganized into four families, spec at
...
**Status of the four, 2026-07-30: three met, one met only in the prototype.** Conditions 1 to 3 are
discharged and their evidence is named above. Condition 4 is the open one, and it is the one wired to
a kill rather than a recut. **The remaining blockers on this cycle are therefore not entry conditions
but the two review gates** — the stakeholder chain on Slice 16 and Principal Designer Mode S on this
document — neither of which has run. See the Status line at the top.
...
- [ ] Given the narrow layout, when the navigation renders, then it exposes **at most five destinations**, and **every one carries a text label**. *(§2.1 layout F7, adopted as a labelled hypothesis. Our **four top-level families** fit this budget with no demotion. Updated 2026-07-30: this previously contrasted our four against "the eleven-item production navigation", which §6.2 entry condition 5 has since settled — production inherits the eleven **reorganized into those same four families**, so the contrast no longer exists and the budget is counted at the top level.)*
...
- **A navigation cardinality device — grouping, a second tier, or a budget-and-overflow — on the Learning Home** (§2.1 layout F1). The cited study's recommendation is to keep every destination visible and apply a structural rule rather than shorten the list, and it is well evidenced: the two benchmarked platforms carrying *more* destinations than our production home both apply a device and both mark current location, while ours does neither. **It is deferred because it is a steady-state information-architecture decision and this PRD builds first-run only**, and because the study's own strongest caveat applies squarely to us: ten of our eleven destinations were never opened, so no grouping can be proposed without a content inventory first. The study's own gate order is content inventory → open card sort in Bahasa Indonesia (n≈15, stratified by facilitated versus self-directed arrival) → first-click test. **Not merely deferred, because it had a live consequence here, and that consequence is now discharged.** ~~the prototype's home carries four destinations and the production home carries eleven, and which one the production first-run home inherits decides whether Slice 14's five-destination budget holds. That is now an owned §15 decision rather than an assumption~~ **Settled 2026-07-30** (§15): production inherits the eleven **reorganized into four top-level families**, and the budget holds at four. **This partially reopens the deferral recorded above** — a purpose-based grouping is precisely the structural device this finding recommends, so the first-run navigation now *has* a device and this entry covers only the steady-state home's full device decision. **The content-inventory caveat is unaffected and still governs the family assignment**, which is carried as an unvalidated judgement from labels behind a card-sort gate, not as an adopted finding
